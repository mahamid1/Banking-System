...
namespace ProjectDraft
{
    class Branches
    {
        ...
        public static List<Branches> GetAll()
        {
         ...
         public static Branches check_if_exist(string UserName,string passworde)
        {
            Branches b = null;
            Connector con = new Connector("Bank.accdb");
            OleDbDataReader result = con.RunSelect("select * from Branches where BrachManeger_UserName='" + UserName + "' and Passworde='" + passworde + "'");
            while (result.Read())
            {
                b=new Branches(int.Parse(result["Branch_Num"].ToString()), result["Branch_Name"].ToString(),
                    result["Branch_Location"].ToString(), result["BrachManeger_UserName"].ToString(),
        ...
        public static List<Branches> GetAllByBranch(int x)
        {
            List<Branches> Branch = new List<Branches>();
            Connector con = new Connector("Bank.accdb");

            OleDbDataReader result = con.RunSelect("select * from Branches where Branch_Num="+x);
            Branches b;
            while (result.Read())
            {
                b = new Branches(int.Parse(result["Branch_Num"].ToString()), result["Branch_Name"].ToString(),
